...
def faceDetection(test_img):
    ...
    face_haar_cascade = cv2.CascadeClassifier(
        'D:\GitHub\FaceRecognition_homeSecurity\HaarCascade\haarcascade_frontalface_default.xml')  # Load haar classifier
    ...
    for path, subdirnames, filenames in os.walk(directory):
        for filename in filenames:
            if filename.startswith("."):
                ...
                continue
            id_path = os.path.basename(path)  # fetching subdirectory names
            img_path = os.path.join(path, filename)  # fetching image path
            print("img_path:", img_path)  # printing path for debugging purpose
            print("id:", id_path)
            ...
            if len(faces_rect) != 1:
                # Since we are assuming only single person images are being fed to train classifier
                continue
            (x, y, w, h) = faces_rect[0]
            # cropping region of interest i.e. face area from grayscale image
            roi_gray = gray_img[y:y + w, x:x + h]
            faces.append(roi_gray)
            face_id.append(int(id_path))
...
def train_classifier(faces, face_id):
    # lBPHFaceRecognizer is used to recognize Faces
...
def draw_rect(test_img, face):
    ...
    cv2.rectangle(test_img, (x, y + h - 20), (x + w, y + h), (0, 255, 0), cv2.FILLED)


# Below function writes name of person for detected label
def put_text(test_img, text, x, y):
    cv2.putText(test_img, text, (x, y), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 0, 0), 1)
